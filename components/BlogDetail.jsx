...
export default function BlogDetail(props) {
  const { title, author, authorImage, image, publishedAt, body, categories } = props;
        ...
        <div className="border-[1px] dark:border-slate-700 border-slate-200 rounded-md">
          ...
          <div className="flex items-center gap-2 p-4">
            {categories.length > 0 && categories.map(category => (
              <div className="bg-slate-300 dark:bg-slate-700 px-2 rounded-sm flex items-center gap-1" key={category}>{category}</div>
            ))}
          </div>
          ...
          <div className="border-[1px] border-slate-200 rounded-md w-full dark:border-slate-700 p-4 sticky top-2 z-30 transition-all">
            ...
            <div className="flex items-center gap-2 mt-4">
              ...
              <div className='flex flex-col'>
                ...
                <span className='text-[.8rem] text-gray-500'>{formatTimestampToDate(publishedAt)}</span>
              </div>
            </div>
          </div>
        </div>
